...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path):
    for each in os.listdir(dir_path_content):
        if os.path.isfile(os.path.join(dir_path_content, each)):
            if each.endswith(".md"):
                generate_page(os.path.join(dir_path_content, each), template_path, os.path.join(dest_dir_path, each))
        else:
            os.makedirs(os.path.join(dest_dir_path, each), exist_ok=True)
            generate_pages_recursive(os.path.join(dir_path_content, each), template_path, os.path.join(dest_dir_path, each))
